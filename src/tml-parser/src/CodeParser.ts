...
export class CodeParser {
  ...
  private _matchValue(value: string) {
    if (this._wrapper.currentValue !== value) {
      throw new SyntaxError(
        `${this._wrapper.currentPosition}- Expected value "${this._wrapper.currentValue}" to be "${value}".`
      );
    }
  }
  ...
  private _moveNext() {
    if (!this._wrapper.moveNext()) {
      throw new SyntaxError(
        `${this._wrapper.currentPosition}- Unexpected end of file.`
      );
    }
  }
  ...
  private _parseProgram(): ProgramContext {
    ...
    if (modules.length === 0) {
      throw new SyntaxError(
        `${position}- A program should have at least one module.`
      );
    }
  ...
  private _parseValues(
    ...
    while (this._wrapper.currentValue !== finishedValue) {
      ...
      if (noComma) {
        throw new SyntaxError(
          `${this._wrapper.currentPosition}- Expected value "${this._wrapper.currentValue}" to be "${finishedValue}".`
        );
      }
      if (includesBlank && this._wrapper.currentValue === 'blank') {
        ...
        if (this._wrapper.currentValue.length !== 1) {
          throw new SyntaxError(
            `${this._wrapper.currentPosition}- The value "${this._wrapper.currentValue}" must have length 1.`
          );
        }
        if (!this._wrapper.currentValue.match(/[a-z|0-9]/)) {
          throw new SyntaxError(
            `${this._wrapper.currentPosition}- The value "${this._wrapper.currentValue}" must be a lowercase character or a number.`
          );
        }
  ...
  private _parseAlphabet(): AlphabetContext {
    ...
    if (alphabet.size === 0) {
      throw new SyntaxError(
        `${position}- The alphabet must have at least one letter.`
      );
    }
  ...
  private _parseModule(): ModuleContext {
    ...
    if (blocks.length === 0) {
      throw new SyntaxError(
        `${position}- A module must have at least one block/command.`
      );
    }
  ...
  private _parseSwitchBlock(): SwitchBlockContext {
    ...
    this._doUntil(
      ...
      () => {
        if (this._wrapper.currentValue === 'if') {
          ...
          throw new SyntaxError(
            `${this._wrapper.currentPosition}- Unexpected start of case: "${this._wrapper.currentValue}".`
          );
        }
      },
      false
    ...
    if (cases.length === 0) {
      throw new SyntaxError(
        `${position}- A switch block must have at least one case.`
      );
    }

    this._moveNext();
    return new SwitchBlockContext(
      CodePosition.combine(position, endPosition),
      cases
    );
  }
  ...
  private _parseIf(): IfCaseContext {
    ...
    if (values.size === 0) {
      throw new SyntaxError(
        `${startPosition}- An if case must apply to at least one letter.`
      );
    }
    this._moveNext();
    ...
    if (blocks.length === 0) {
      throw new SyntaxError(
        `${position}- An if case must have at least one command.`
      );
    }
  ...
  private _parseWhile(): WhileCaseContext {
    ...
    if (values.size === 0) {
      throw new SyntaxError(
        `${startPosition}- A while case must apply to at least one letter.`
      );
    }
    this._moveNext();
    ...
    if (this._wrapper.currentValue === '}') {
      throw new SyntaxError(
        `${startPosition}- A while case must have at least one command.`
      );
    }
    ...
    if (['move', 'changeto'].includes(this._wrapper.currentValue)) {
      throw new SyntaxError(
        `${this._wrapper.currentPosition}- A while case cannot have more than one core block.`
      );
    }
  ...
  private _parseBasicBlock(): BasicBlockContext {
    ...
    if (startPosition === endPosition) {
      throw new SyntaxError(
        `${startPosition}- Invalid basic command "${this._wrapper.currentValue}".`
      );
    }
  ...
  private _parseCoreBlock(): CoreBasicBlockContext {
    ...
    if (startPosition === endPosition) {
      throw new SyntaxError(
        `${startPosition}- Invalid core command "${this._wrapper.currentValue}".`
      );
    }
  ...
  private _parseChangeTo(): ChangeToContext {
    ...
    const value =
      this._wrapper.currentValue === 'blank' ? '' : this._wrapper.currentValue;
  ...
  private _parseMove(): MoveContext {
    ...
    switch (this._wrapper.currentValue) {
      ...
      default:
        throw new SyntaxError(
          `${this._wrapper.currentPosition}- Invalid direction "${this._wrapper.currentValue}".`
        );
    }
    const endPosition = this._wrapper.currentPosition;
    const position = CodePosition.combine(startPosition, endPosition);
