...
export class CodeWrapper {
  ...
  private _isCharDelimiter(index: number): boolean {
    return (
      this._isCharWhitespace(index) ||
      CodeWrapper._delimiters.includes(this.code[index])
    );
  }
  ...
  private get _nextEndCol(): number {
    ...
    if (CodeWrapper._delimiters.includes(this.code[this._startIndex])) {
